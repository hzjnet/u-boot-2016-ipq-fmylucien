...
def generate_code_hash(env, elf_in_file_name):
   ...
   for i in range(elf_header.e_phnum):
      ...
      if (curr_phdr.p_align == 8 and
         ...
         hash_data = struct.unpack("I" * (curr_phdr.p_filesz // DP_HASH_SIZE), hash_data)
   ...
   for i in range(elf_header.e_phnum):
      ...
      if (curr_phdr.p_align == DP_CODE_ALIGN and
         ...
         if (code_seg_idx != -1):
            raise RuntimeError('Multiple code segments match for: ' + str(code_seg_pages) + ' pages')
         code_seg_idx = i
        ...
        if bracket_counter == 0:
           ...
           # Catch NameError and attempt to locate start address from dictionary
              ...
              if start_addr in global_dict:
                 start_addr_final = global_dict[start_addr]
              else:
...
def verify_elf_header(elf_header):
   # Convert single byte to bytes object for comparison in Python 3
   if (bytes([elf_header.e_ident[ELFINFO_MAG0_INDEX]]) != ELFINFO_MAG0) or \
      (bytes([elf_header.e_ident[ELFINFO_MAG1_INDEX]]) != ELFINFO_MAG1) or \
      (bytes([elf_header.e_ident[ELFINFO_MAG2_INDEX]]) != ELFINFO_MAG2) or \
      (bytes([elf_header.e_ident[ELFINFO_MAG3_INDEX]]) != ELFINFO_MAG3) or \
      ((bytes([elf_header.e_ident[ELFINFO_CLASS_INDEX]]) != ELFINFO_CLASS_64) and \
       (bytes([elf_header.e_ident[ELFINFO_CLASS_INDEX]]) != ELFINFO_CLASS_32)) or \
      (bytes([elf_header.e_ident[ELFINFO_VERSION_INDEX]]) != ELFINFO_VERSION_CURRENT):
...
def insert_SBL1_magicCookie (env, target):
    ...
    if (length <= VIRTUAL_BLOCK_SIZE):
        ...
        while length > VIRTUAL_BLOCK_SIZE:
            ...
            mc1 = bytes([int(a[8:10],16)]) + bytes([int(a[6:8],16)]) + bytes([int(a[4:6],16)]) + bytes([int(a[2:4],16)])

            b = str(hex(MAGIC_NUM))
            mc2 = bytes([int(b[8:10],16)]) + bytes([int(b[6:8],16)]) + bytes([int(b[4:6],16)]) + bytes([int(b[2:4],16)])

            c = str(hex(SBL_VIRTUAL_BLOCK_MAGIC_NUM))
            mc3 = bytes([int(c[8:10],16)]) + bytes([int(c[6:8],16)]) + bytes([int(c[4:6],16)]) + bytes([int(c[2:4],16)])
